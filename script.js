window.onload = function () {
  ...
  const clockSkins = [
    ...
    },
    {
      ...
      name: "Пурпурний",
      apply: () => {
        clock.style.borderColor = "#8b5cf6";
  ...
  const handSkins = [
    {
      ...
      name: "Темно-сині",
      apply: () => {
        document.querySelectorAll(".hand").forEach(h => h.style.background = "#1e3a8a");
      }
    },
    {
      id: "neon",
      name: "Неонові",
      apply: () => {
        document.querySelectorAll(".hand").forEach(h => h.style.background = "#0ea5e9");
      }
    },
    {
      id: "pixel",
      name: "Піксельні",
      apply: () => {
        document.querySelectorAll(".hand").forEach(h => h.style.background = "linear-gradient(#fff,#aaa)");
      }
    },
    {
      id: "chrome",
      name: "Хром",
      apply: () => {
        document.querySelectorAll(".hand").forEach(h => h.style.background = "linear-gradient(90deg,#ddd,#888,#ddd)");
      }
    }
  ];
  ...
  function updateClockHands() {
    ...
    document.querySelectorAll(".second").forEach(hEl =>
      hEl.style.transform = `translateX(-50%) rotate(${s * 6}deg)`
    );

    document.querySelectorAll(".minute").forEach(hEl =>
      hEl.style.transform = `translateX(-50%) rotate(${m * 6 + s * 0.1}deg)`
    );

    document.querySelectorAll(".hour").forEach(hEl =>
      hEl.style.transform = `translateX(-50%) rotate(${h * 30 + m * 0.5}deg)`
    );
  }
...
// === РЕВЕРБ: УТРИМАННЯ ДЛЯ АКТИВАЦІЇ ===
...
const startReverbHold = () => {
  ...
  const accelInterval = setInterval(() => {
    ...
    // Швидкість та напрямок
    document.querySelectorAll("#reverbClock .hand").forEach(hand => {
      const baseSpeed = 360 * (reverbTime * 2 + 5);
      const direction = Math.random() > 0.5 ? 1 : -1;
      const rotation = direction * baseSpeed * 10;

      hand.style.animationDuration = `${Math.max(0.3, 3 - reverbTime * 0.25)}s`;
      hand.style.setProperty("--rand", `${rotation}deg`);
    });

    // Кольоровий ефект обідка
...
reverbClock.addEventListener("touchstart", e => {
  e.preventDefault();
  startReverbHold();
});
